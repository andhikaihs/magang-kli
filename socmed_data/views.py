    ...
    for data in SocialMediaData.objects.filter(ue1=ue1, created_at__range=(agenda_start, agenda_end)):
        if data.social_media == 'Instagram':
            ...
            for post_url, caption, like, comment, viewer in zip(post_urls, captions, likes, comments, viewers):
                # Convert the captions into a list
                caption_list = [caption.strip() for caption in caption.split('\n') if caption.strip()]

                for caption_text in caption_list:
                    similarity = calculate_similarity(caption_text, agenda_setting.pesan_kunci, agenda_setting.sub_pesan_kunci)
